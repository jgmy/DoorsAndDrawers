...
class Furniture {
  ...
  int receiveItem(Item item) {
    ...
    case FURNCOPIER:
      ...
      break;
    case FURNCOFFEE:
      if (item.itype!=ITEMCUP &&  item.itype!= ITEMJARCOFFEE ) {
        return -1;
      }
      break;
    }  
    ...
    case FURNMAC:
      ...
      final int[] copydst={FURNCOPIER};
      if (insertAtRandomFurn(copydst, ITEMMAP)==null) {
    ...
    case FURNCOPIER:
      ...
      case 1:
        ...
        /* EXPAND -- Print a random MAP, CLUE or PASSWORD */
          int n=int (random(4));
          n=(n==3 ? ITEMHINT : (n==2 ? ITEMMAP : (n==1 ? ITEMPASSWORD : ITEMSECRETRECIPE)));
          
         for (int f=0;f<this.items.length;f++) 
           if (this.items[f] ==null || this.items[f].itype==ITEMEMPTY) this.items[f]=new Item(n);
        break;
      case 2:
        tmpDialog("NO TONER");
        break;
    ...
    case FURNCOFFEE:
      if ((item.itype==ITEMCUP) && this.containsItem(ITEMJARCOFFEE)){
        ...
        item.name="A cup of hot coffee";
        item.image=itemCupCoffee;
      } else if ( (item.itype==ITEMJARCOFFEE && this.containsItem(ITEMCUP)) ||
            (this.containsItem(ITEMCUP) && this.containsItem(ITEMJARCOFFEE))){
        Item destcoffee=this.getFirstItem(ITEMCUP);
          destcoffee.itype=ITEMCUPCOFFEE;
          destcoffee.image=itemCupCoffee;
          destcoffee.name="A cup of hot coffee";
  ...
  int itemIndex=0;
  /* Return first item of type itype. Use with caution */
  Item getFirstItem(int itype) {
  ...
  /* Return first item of type itype. Use with caution */
  Item getNextItem(int itype) {
    if (this.items==null) return null;
    for (int f=this.itemIndex; f<this.items.length; f++) {
      this.itemIndex=f;
      if ((this.items[f]!=null) &&  (this.items[f].itype==itype) ) {
        return this.items[f];
      }
